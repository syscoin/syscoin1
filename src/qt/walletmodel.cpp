...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel,
		QObject *parent) :
		QObject(parent), wallet(wallet), optionsModel(optionsModel), addressTableModel(
				0), transactionTableModel(0), aliasTableModelMine(0), aliasTableModelAll(
				0), certTableModelMine(0), certTableModelAll(0), offerTableModelMine(
				0), offerTableModelAll(0), offerTableModelAccept(0), offerTableModelMyAccept(
				0), cachedBalance(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(
				0), cachedNumTransactions(0), cachedEncryptionStatus(
				Unencrypted), cachedNumBlocks(0) {
...
void WalletModel::updateAddressBook(const QString &address,
		const QString &label, bool isMine, int status) {
...
foreach(const SendCoinsRecipient &rcp, recipients)
	...
	bool fCreated = wallet->CreateTransaction(vecSend, wtx, keyChange,
			nFeeRequired, strFailReason, coinControl);
...
static void NotifyAliasListChanged(WalletModel *walletmodel, CWallet *wallet,
	const CTransaction *tx, ChangeType status) {
...
OutputDebugStringF("NotifyAliasListChanged status=%i\n", status);
QMetaObject::invokeMethod(walletmodel, "updateAlias", Qt::QueuedConnection);

}

static void NotifyCertListChanged(WalletModel *walletmodel, CWallet *wallet,
	const CTransaction *tx, ChangeType status) {
std::vector<std::vector<unsigned char> > vvchArgs;
int op, nOut;
...
OutputDebugStringF("NotifyCertListChanged status=%i\n", status);
QMetaObject::invokeMethod(walletmodel, "updateCert", Qt::QueuedConnection);
}
static void NotifyOfferListChanged(WalletModel *walletmodel, CWallet *wallet,
	const CTransaction *tx, ChangeType status) {
std::vector<std::vector<unsigned char> > vvchArgs;
int op, nOut;
...
OutputDebugStringF("NotifyOfferListChanged status=%i\n", status);
...
wallet->NotifyAliasListChanged.connect(
	boost::bind(NotifyAliasListChanged, this, _1, _2, _3));
wallet->NotifyOfferListChanged.connect(
	boost::bind(NotifyOfferListChanged, this, _1, _2, _3));
wallet->NotifyCertListChanged.connect(
	boost::bind(NotifyCertListChanged, this, _1, _2, _3));
...
wallet->NotifyAddressBookChanged.disconnect(
	boost::bind(NotifyAddressBookChanged, this, _1, _2, _3, _4, _5));
wallet->NotifyAliasListChanged.disconnect(
	boost::bind(NotifyAliasListChanged, this, _1, _2, _3));
wallet->NotifyOfferListChanged.disconnect(
	boost::bind(NotifyOfferListChanged, this, _1, _2, _3));
wallet->NotifyCertListChanged.disconnect(
	boost::bind(NotifyCertListChanged, this, _1, _2, _3));
